...
export default function Error({ error, reset }) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background-dark p-4 animate-fade-in">
      <div className="max-w-lg w-full bg-component-dark rounded-lg p-8 border border-red-500/20 shadow-xl animate-scale-in">
        <div className="flex flex-col items-center text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full">
            <button
              onClick={reset}
              className="flex-1 px-4 py-2 bg-primary text-black rounded-lg font-bold hover:bg-yellow-400 transition-all hover-lift button-press"
            >
              Try Again
            </button>
            <button
              onClick={() => window.location.reload()}
              className="flex-1 px-4 py-2 bg-white/10 text-white rounded-lg font-bold hover:bg-white/20 transition-all hover-lift button-press"
            >
              Reload Page
            </button>
            <Link
